import {GET_COURSE_SUCCESS,GET_COURSE_FAIL,GET_COURSE_INSTR_SUCCESS,GET_COURSE_INSTR_FAIL,ADD_COURSE_SUCCESS,
...
export const get_Course = () => async(dispatch) => {
   ...
   /*  export const get_Course_by_Instructor = (id) => async(dispatch) => {
        try {
            const res= await axios.get(`/Courses/${id}`)
    
            dispatch({type:GET_COURSE_INSTR_SUCCESS , payload:res.data})
        } 
        catch (error) {
            dispatch({type:GET_COURSE_INSTR_FAIL,payload:error})
        }
        } */
